...
public class TransactionControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  @Autowired private AccountPinSecurityRepository accountPinSecurityRepository;
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.balanceAfter")
                .value(account.getBalance().add(request.getAmount()).doubleValue()))
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.description")
                .value(containsString(String.join(" ", "status is", account1.getStatus().name()))));
  }
  ...
  public void shouldRegisterDepositWithPinInvalid() throws Exception {
    ...
    TransactionRequest pinMaximumSize =
        TransactionDataProvider.createMockTransactionRequest("12345");
    ...
    List<TransactionRequest> invalidRequests =
        List.of(pinMinimumSize, pinMaximumSize, pinLetters, pinBlankSpace, pinNull);
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      if (i < 4) {
        result
            ...
            .andExpect(
                jsonPath("$.description")
                    .value(containsString(remainingAttempts[i - 1] + " attempts")));
      } else {
        result
            ...
            .andExpect(jsonPath("$.description").value(containsString("temporarily locked")))
            .andExpect(
                jsonPath("$.description")
                    .value(
                        containsString(
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      if (i < 4) {
        ...
        result
            ...
            .andExpect(jsonPath("$.code").value(HttpStatus.LOCKED.value()))
            .andExpect(jsonPath("$.name").value(HttpStatus.LOCKED.getReasonPhrase()))
            .andExpect(jsonPath("$.description").value(containsString("permanently blocked")));
      }
    }
  }
  ...
  public void should_return_401_when_request_is_unauthenticated() throws Exception {
    ...
    TransactionRequest request =
        TransactionRequest.builder().amount(BigDecimal.valueOf(100.00)).pin("1234").build();
  ...
  @Test
  public void should_return_403_when_account_does_not_belong_to_authenticated_user()
      throws Exception {
  ...
  public void shouldReturn400WhenUuidFormatIsInvalid() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/accounts/{accountId}/transactions", TransactionDataProvider.INVALID_UUID)
                .with(user(customerId.toString()).roles(UserRole.CUSTOMER.name())))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value(HttpStatus.BAD_REQUEST.value()))
        .andExpect(jsonPath("$.name").value(HttpStatus.BAD_REQUEST.getReasonPhrase()))
  ...
  public void shouldReturn401WhenIsNotAuthenticated() throws Exception {
    mockMvc
        .perform(get("/api/accounts/{accountId}/transactions", account.getId()))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value(HttpStatus.UNAUTHORIZED.value()))
        .andExpect(jsonPath("$.name").value(HttpStatus.UNAUTHORIZED.getReasonPhrase()))
        .andExpect(jsonPath("$.description").exists());
  ...
  public void shouldReturn200WithDefaultRecordCountAndDatabaseEntries() throws Exception {
    ...
    List<TransactionEntity> dataMock =
        TransactionDataProvider.createMockTransactions(registersMock, account, now);
    ...
    TransactionsHistoryResponse response =
        mapper.readValue(json, TransactionsHistoryResponse.class);
  ...
  @Test
  public void shouldReturn200WhenSingleObjectRequestedAndPageCountEqualsDatabaseObjects()
      throws Exception {
    UUID customerId = account.getCustomerId();
    int registersMock = 120;
  ...
  public void shouldReturn200WhenPaginationIsTestedInSystem() throws Exception {
    ...
    Instant now = Instant.now();
    List<TransactionEntity> dataMock =
        TransactionDataProvider.createMockTransactions(totalTransactions, account, now);
    transactionRepository.saveAll(dataMock);
    ...
    while (true) {
      ...
      assertEquals(
          pageNumber, response.getPage(), "La página devuelta debe coincidir con la solicitada");
      int expectedContentSize = Math.min(pageSize, totalTransactions - totalFetched);
      assertEquals(
          expectedContentSize, response.getContent().size(), "Content size debe ser correcto");
  ...
  public void shouldReturn200WhenFilteringByFromDate() throws Exception {
    ...
    response.getContent().forEach(tx -> assertFalse(tx.getTimestamp().isBefore(fromDate)));
  ...
  public void shouldReturn200WhenFilteringByToDate() throws Exception {
    ...
    response.getContent().forEach(tx -> assertTrue(tx.getTimestamp().isBefore(toDate)));
  ...
  public void shouldReturn200WhenFilteringByFromDateAndToDate() throws Exception {
    ...
    response
        ...
        .forEach(
            tx ->
                assertTrue(
                    !tx.getTimestamp().isBefore(fromDate) && !tx.getTimestamp().isAfter(toDate)));
