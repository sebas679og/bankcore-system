...
public class TransferControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  @Test
  public void shouldRegisterTransferToExternalAccountSuccessfullyWhenDescriptionIsNull()
      throws Exception {
    UUID customerId = sourceAccount.getCustomerId();
  ...
  public void shouldFailWhenDestinationAccountIsInactive() throws Exception {
    ...
    AccountEntity accountInactive =
        accountRepository.save(AccountDataProvider.createMockAccountStatusInactive());
  ...
  public void shouldRegisterTransferWithPinInvalid() throws Exception {
    ...
    TransferRequest pinMinimumSize =
        TransferAccountProvider.createMockTransferRequest(
            sourceAccount.getId(), "12", destinationAccount.getAccountNumber());

    TransferRequest pinMaximumSize =
        TransferAccountProvider.createMockTransferRequest(
            sourceAccount.getId(), "12345", destinationAccount.getAccountNumber());

    TransferRequest pinLetters =
        TransferAccountProvider.createMockTransferRequest(
            sourceAccount.getId(), "12mp", destinationAccount.getAccountNumber());

    TransferRequest pinBlankSpace =
        TransferAccountProvider.createMockTransferRequest(
            sourceAccount.getId(), "", destinationAccount.getAccountNumber());

    TransferRequest pinNull =
        TransferRequest.builder()
            .sourceAccountId(sourceAccount.getId())
            .destinationAccountNumber(destinationAccount.getAccountNumber())
    ...
    List<TransferRequest> invalidRequests =
