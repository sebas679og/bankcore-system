...
public class ProfileControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  @Autowired private UserManagementImpl userManagement;
    ...
    mockMvc
        .perform(
            get("/api/customers/me")
                .with(user(savedUser.getId().toString()).roles(DataProvider.CUSTOMER_ROLE)))
        .andDo(print())
  ...
  void shouldReturn200WhenAdminRequestsCustomer() throws Exception {
    mockMvc
        .perform(get("/api/customers/{id}", savedUser.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        ...
        .andExpect(
            jsonPath("$.fullName")
                .value(String.join(" ", savedUser.getFirstName(), savedUser.getLastName())))
  ...
  void shouldReturn400WhenServiceRequestsCustomerPathInvalidFormat() throws Exception {
    mockMvc
        .perform(get("/api/customers/{id}", DataProvider.INVALID_UUID))
  ...
  void shouldReturn404WhenServiceRequestsCustomerNotFound() throws Exception {
    mockMvc
        .perform(get("/api/customers/{id}", DataProvider.UUID))
        .andExpect(status().isNotFound())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.code").value(HttpStatus.NOT_FOUND.value()))
        .andExpect(jsonPath("$.name").value(HttpStatus.NOT_FOUND.getReasonPhrase()))
  ...
  void shouldReturn200WhenServiceRoleCustomerExist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/customers/{id}/validate", savedUser.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldReturn200WhenServiceRoleCustomerNotExist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/customers/{id}/validate", DataProvider.UUID))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldReturn400WhenServiceRolePathFormatIncorrect() throws Exception {
    ...
    mockMvc
        .perform(get("/api/customers/{id}/validate", DataProvider.INVALID_UUID))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.code").value(HttpStatus.BAD_REQUEST.value()))
        .andExpect(jsonPath("$.name").value(HttpStatus.BAD_REQUEST.getReasonPhrase()))
  ...
  void shouldReturnTrueWhenProvidedPinMatchesStoredPin() throws Exception {
    ...
    PinValidateRequest request =
        DataProvider.createMockPinValidate(DataProvider.createMockUser().getAtmPin());

    mockMvc
        .perform(
            post("/api/customers/{id}/validate-pin", savedUser.getId())
  ...
  void shouldReturn404WhenUserDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/customers/{id}/validate-pin", DataProvider.UUID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isNotFound())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldReturn400WhenCustomerIdIsInvalidUuid() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/customers/{id}/validate-pin", DataProvider.INVALID_UUID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldReturn400WhenPinRequestValidationFails() throws Exception {
    ...
    PinValidateRequest pinNull = PinValidateRequest.builder().pin(null).build();

    List<PinValidateRequest> invalidRequests =
        List.of(pinMinimunSize, pinMaximumSize, pinLetters, pinSameDigits, pinBlankSpace, pinNull);
